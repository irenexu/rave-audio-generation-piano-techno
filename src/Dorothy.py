...
class RAVEPlayer(AudioDevice):
    def __init__(self, model_path, new_frame=lambda: 0, fft_size=512, buffer_size=2048, sr=44100, latent_dim=128):
...
class MusicAnalyser:
    ...
    def load_rave(self, model_path="vintage.ts",fft_size=1024, buffer_size=2048, sr = 44100, latent_dim= 128):
        self.audio_device = RAVEPlayer(model_path=model_path, new_frame = self.new_frame, buffer_size=buffer_size, sr=sr, fft_size=fft_size, latent_dim=latent_dim)
    ...
    def update_rave_latent(self, new_latent):
        if isinstance(self.audio_device, RAVEPlayer):
            if isinstance(new_latent, torch.Tensor):
                if self.audio_device.current_latent.shape == new_latent.shape:
                    self.audio_device.current_latent = new_latent
                else:
                    print(f'new latent shape {new_latent.shape} does not match shape of existing latent {self.audio_device.current_latent.shape} ')
            else:
                print(f'the latent passed into this function should be a Torch tensor, not a {type(new_latent)}')
        else:
            print(f'audio device {type(self.audio_device)} is not a RAVEPlayer, therefore latent cannot be updated')
